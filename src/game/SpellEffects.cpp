...
void Spell::EffectLeapForward(SpellEffectIndex eff_idx)
{
    ...
    {
		float dis = m_caster->Unit::GetSpellRadiusForTarget(unitTarget, sSpellRadiusStore.LookupEntry(m_spellInfo->EffectRadiusIndex[eff_idx]));
        //float dis = GetSpellRadius(sSpellRadiusStore.LookupEntry(m_spellInfo->EffectRadiusIndex[eff_idx]));
		float orientation = unitTarget->GetOrientation(), step = dis/10.0f;
		...
		destx = ox + dis * cos(orientation);
		desty = oy + dis * sin(orientation);
		ground = unitTarget->GetMap()->GetHeight(destx,desty,MAX_HEIGHT,true);
		floor = unitTarget->GetMap()->GetHeight(destx,desty,oz, true);
		destz = fabs(ground - oz) <= fabs(floor - oz) ? ground : floor;

		bool col = VMAP::VMapFactory::createOrGetVMapManager()->getObjectHitPos(mapid,ox,oy,oz+0.5f,destx,desty,destz+0.5f,destx,desty,destz,-0.5f);

		if (col) // We had a collision!
    {
        destx -= 0.6 * cos(orientation);
        desty -= 0.6 * sin(orientation);
        dis = sqrt((ox-destx)*(ox-destx) + (oy-desty)*(oy-desty));
        step = dis/10.0f;
    }
