    ...
    void UpdateAI(const uint32 diff)
    {
        if (isTeleported)
        {
            if (Teleport_Timer < diff)
            {
                ...
                m_creature->SendMonsterMove(LastX, LastY, LastZ, SPLINETYPE_STOP, m_creature->GetSplineFlags(), 1);
        ...
        if (Teleport_Timer < diff)
        {
            ...
            m_creature->SendMonsterMove(TELE_X, TELE_Y, TELE_Z, SPLINETYPE_STOP, m_creature->GetSplineFlags(), 1);
            isTeleported = true;
            SecondPhaseCounter = 0;
            SecondPhase_Timer = 0;
...
struct MANGOS_DLL_DECL boss_nothAI : public ScriptedAI
{
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (m_uiSummonTimer < uiDiff)
        {
            ...
            for(uint8 i = 0; i < 6; ++i)
                m_creature->SummonCreature(NPC_PLAGUED_WARRIOR, 2684.804f, -3502.517f, 261.313f, 0.0f, TEMPSUMMON_TIMED_OR_DEAD_DESPAWN, 80000);
